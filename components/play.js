...
const processPlay = (gameState, basicPlay, modifiers, runnerResults) => {
  ...
  if (/^[1-9](\(([1-9]|B)*\))?$/.test(basicPlay) && !modifiers.check(/^FO$/) && !modifiers.check(/^B*G(\+|-)?$/)) flyBallOut(gameState, basicPlay, modifiers, runnerResults); // eg. 8/F78 (fly ball out)
  ...
  else if (modifiers.check(/^B*GDP$/) || (modifiers.check(/^DP$/) && !/^OA$/.test(basicPlay) && !/^K([1-9]*)?(\+.*)?$/.test(basicPlay) && !/^(PO)[1-3](\(.*\))?$/.test(basicPlay) && !/(CS([2-3]|H)|POCS([2-3]|H))(\([1-9]*E*[1-9]*(\/TH)?\))?(\(UR\))?$/.test(basicPlay))) groundBallOut(gameState, basicPlay, modifiers, runnerResults, 2); // eg., 64(1)3/GDP/G6 (ground into double play)
    ...
    log.error('Something else', basicPlay, modifiers);
    process.exit();
  }
};
